...
{
    BOOL bRet = FALSE;
    UINT uiCount, uiRawDevCount;
    DWORD cbNeeded;
    PSMI_DEVBLK_ENTRY pEntry;
    ...
    uiCount = GetDevBlkCount(&uiRawDevCount);
    cbNeeded = uiCount * sizeof(SMI_DEVBLK_INFO);
    if (cbBuf < cbNeeded)
    {
        if (pcbNeeded)
        {
    ...
    else
    {
        ...
        {
            cbNeeded = 0;
            for (UINT i = 0; i < uiRawDevCount; i++)
            {
                for (UINT j = 0; j < 129; j++)
                {
                    pEntry = GetDevBlkEntry(i, j);
                    if (pEntry)
                    {
                        assert(sizeof(SMI_DEVBLK_INFO) == sizeof(SMI_DEVBLK_ENTRY));
                        PSMI_DEVBLK_INFO pInfo = (PSMI_DEVBLK_INFO)pDevBlkArray;
                        memcpy(pDevBlkArray + cbNeeded, pEntry, sizeof(SMI_DEVBLK_INFO));
                        cbNeeded += sizeof(SMI_DEVBLK_INFO);
                    }
                }
            }
            if (pcbNeeded)
    ...
    _Out_ LPDWORD pcReturned)
{
    ...
    cbNeeded = 0;
    uiCount = GetRawDevCount();
    cbNeeded = uiCount * sizeof(SMI_DRIVE_INFO);
    if (cbBuf < cbNeeded)
    {
        if (pcbNeeded)
        {
    ...
    else
    {
        if (pDriveInfoArray)
        {
            ...
            {
                PSMI_RAWDEV_ENTRY pRawDevEntry = GetRawDevEntry(i);
                if (pRawDevEntry)
                {   
                    PSMI_DRIVE_INFO pEnumDriveInfo = (PSMI_DRIVE_INFO)pDriveInfoArray;
                    //pEnumDriveInfo->u32DeviceNumber = pRawDevEntry->DriveInfo.u32DeviceNumber;
                    ////pEnumDriveInfo->MediaType = pRawDevEntry->DriveInfo.diskGeometry.Geometry.MediaType;
                    //pEnumDriveInfo->DiskSize = pRawDevEntry->DriveInfo.diskGeometry.DiskSize;
                    cbNeeded += sizeof(SMI_DRIVE_INFO);
                }
            }
            if (pcbNeeded) 
            { 
